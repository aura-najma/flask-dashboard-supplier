...
def index():
    ...
    top_product_query = (
        db.session.query(Product.nama_product, func.sum(OrderDetail.kuantitas).label('total_terjual')) # <-- PERBAIKAN 1
        .join(OrderDetail, OrderDetail.id_product == Product.id_product)
        .group_by(Product.id_product)
        .order_by(func.sum(OrderDetail.kuantitas).desc()) # <-- PERBAIKAN (konsistensi)
        .first()
    )
    ...
    total_pendapatan_query = (
        db.session.query(func.sum(Product.harga * OrderDetail.kuantitas)) # <-- PERBAIKAN 2
        .join(OrderDetail, OrderDetail.id_product == Product.id_product)
        .scalar()
    )
